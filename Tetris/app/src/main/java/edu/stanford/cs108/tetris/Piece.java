...
public class Piece {
	...
	*/
	public Piece(TPoint[] points) {
		this.body = points;
		this.skirt = setSkirt(points);
		this.width = this.skirt.length;
		this.height = setHeight(points);
	}
	...
	// Returns the skirt arrays according to TPoint[] array
	private int[] setSkirt(TPoint[] points) {
		List<Integer> result = new ArrayList<Integer>();
		List<Integer> size = new ArrayList<Integer>();  // record the size of the returning skirt
		for(int i = 0; i<points.length; i++) {
			result.add(100);
		}
		for(int i = 0; i<points.length; i++) {
			if(!size.contains(points[i].x)) size.add(points[i].x);
			if(result.get(points[i].x) > points[i].y) result.set(points[i].x, points[i].y);
		}
		int[] skirt = new int[size.size()];
		for(int j = 0; j<skirt.length; j++) {
			skirt[j] = result.get(j);
		}
		return skirt;
	}
	
	// Returns the height according to TPoint[] array
	private int setHeight(TPoint[] points) {
		List<Integer> height = new ArrayList<Integer>();
		for(int i = 0; i<points.length; i++) {
			if(!height.contains(points[i].y)) height.add(points[i].y);
		}
		return height.size();
	}
	...
	*/
	...
	public Piece computeNextRotation() {
		...
		for(int i =0 ; i<this.body.length; i++) {
			...
			int newX = this.getHeight() - 1 - this.body[i].y;
	...
	public boolean equals(Object obj) {
	    ...
	    for(int i=0; i<this.body.length; i++) {
	    	for(int j=0; j<other.body.length; j++) {
	    		if(this.body[i].equals(other.body[j])) num++;
	    	}
	    }		
	...
	*/
	private static Piece makeFastRotations(Piece root) {
	    Piece rotation = root;
	    while(!rotation.computeNextRotation().equals(root)) {
